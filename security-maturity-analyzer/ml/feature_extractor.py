...
class LogFeatureExtractor:
    ...
    def extract(self, entry: LogEntry) -> np.ndarray:
        """Alias de transform_one() para compatibilidad con el Autoencoder."""
        return self.transform_one(entry)

    def extract_numeric(self, entry: LogEntry) -> np.ndarray:
        """Retorna solo las 13 features numéricas (para el LSTM)."""
        return self._numeric_features(entry)
