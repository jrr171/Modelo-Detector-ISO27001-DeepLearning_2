...
class DLPipeline:
    ...
    def run(
        ...
        is_anom = self.autoencoder.predict(entries)
        ...
        res.lstm_train_acc  = self.lstm.train_acc_
        res.lstm_val_acc    = self.lstm.val_acc_
        res.lstm_summary    = self.lstm.summary()

        # ── MLP Classifier ───────────────────────────────────────────────
        _lvl, _conf, _probs = self.classifier.predict_with_confidence(domain_stats)
        res.dl_predicted_level = _lvl
        res.dl_level_name      = __import__('rules.iso27001_controls', fromlist=['MATURITY_LEVELS']).MATURITY_LEVELS[_lvl]["name"]
        res.dl_confidence      = _conf
        res.dl_probabilities   = _probs
        res.mlp_train_loss     = self.classifier.train_losses_
        res.mlp_val_loss       = self.classifier.val_losses_
        res.mlp_train_acc      = self.classifier.train_acc_
        res.mlp_val_acc        = self.classifier.val_acc_
